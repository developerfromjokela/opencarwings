...
def handle_cp(xml_data, files):
    if 'send_data' in xml_data['service_info']['application']:
        ...
        if req_id == 281:
            location_center = parse_std_location_precise(file_content[7], int.from_bytes(file_content[13:17], "big"), int.from_bytes(file_content[9:13], "big"))
            ...
            for charger in chargers_info:
                ...
                cpinfo_obj = {
                    'poi_id': charger['ID'],
                    'name': charger['AddressInfo'].get('Title', 'Charging Station') or "Charging Station",
                    'code': '',
                    'county': charger["AddressInfo"].get("Town", '') or "",
                    'region': charger["AddressInfo"].get('StateOrProvince', '') or "",
                    'city': charger["AddressInfo"].get("Town", '') or "",
                    'town': charger["AddressInfo"].get("Postcode", '') or "",
                    'meta1': '',
                    'meta2': '',
                    'meta3': '',
                    'lat': charger["AddressInfo"]["Latitude"],
                    'lon': charger["AddressInfo"]["Longitude"],
                    'address': charger["AddressInfo"].get("AddressLine1", "") or "",
                    'mesh_id': 65535,
                    'phone': phone_num,
                    'sites': [],
                    'stations': [
