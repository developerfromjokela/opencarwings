...
def handle(bin_data: bytes, acp_data: dict, car: Car, source_id: int, destination_id: int) -> bytes:
    ...
    if result_code == 1 and data_type == 1:
        ...
        new_alert.type = 95
        new_alert.additional_data = _("Door Lock/Unlock function is disabled in TCU")
        new_alert.car = car
        ...
        sync_to_async(
            send_vehicle_alert_notification(
                car,
                _("Door Lock/Unlock function is disabled in TCU"),
                _("Could not execute lock/unlock")
            )
        ...
        new_alert = AlertHistory()
        new_alert.type = 95
        message = _("Could not execute lock/unlock, please try again later")
        ...
        sync_to_async(
            send_vehicle_alert_notification(
                car,
                _("CANBUS failure when executing lock/unlock"),
                _("Could not execute lock/unlock")
            )
            , thread_sensitive=False)
    elif data_type == 0:
